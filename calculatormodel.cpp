...
// Validate string
void CalculatorModel::ValidateString(const std::string& problem)
{
	int brackets = 0;
	for (std::size_t step = 0; step < problem.length(); step++)
	{
		if (isdigit(problem[step]) && problem[step + 1] == '.' &&
			isdigit(problem[step + 2]) && problem[step + 3] == '.')
		{
			error_ = Error(INVALID_DOT, step + 1);
		}
		if (problem[step] == '(')
		{
			brackets++;
			...
			{
				error_ = Error(EMPTY_BRACKETS, step + 1);
			}
		...
		if (problem[step] == ')')
		{
			...
			{
				error_ = Error(INVALID_EXPRESSION, step + 1);
			}
		}
	}
	if (brackets != 0)
	{
		error_ = Error(INVALID_COUNT_BRACKETS, 0);
	}
}

//  Determine is it unar plus or minus
int CalculatorModel::IsUnar(const std::string& problem, const int step)
{
	if (stack_.empty())
	{
		return 0;
...
void CalculatorModel::ParseNumber(const std::string& problem, int& step)
{
	int endIndex = step;
	...
	stack_.emplace(number, kNumber, kLow);
	step = endIndex - 1;
}
...
void CalculatorModel::ParseString(std::string& problem)
{
	...
	// и мне не нужно в каждой проверке continue ставить
	{
		...
		{
			stack_.emplace(-1, kNumber, kLow);
			stack_.emplace(0, kMultiply, kUnary);
		}
		else if (problem[step] == '+' && step == 0)
		{
			stack_.emplace(1, kNumber, kLow);
		...
		{
			stack_.emplace(0, kPlus, kLow);
		}
		else if (problem[step] == '-')
		{
			stack_.emplace(0, kMinus, kLow);
		}
		else if (problem[step] == '*')
		{
			stack_.emplace(0, kMultiply, kMiddle);
		}
		else if (problem[step] == '/')
		{
			stack_.emplace(0, kDivide, kMiddle);
		}
		else if (problem[step] == '!')
		{
			stack_.emplace(0, kFactorial, kUnary);
		}
		else if (problem.compare(step, 3, "mod") == 0)
		{
			stack_.emplace(0, kMod, kMiddle);
			step += 2;
		}
		else if (problem[step] == '^')
		{
			stack_.emplace(0, kPow, kUnary);
		}
		else if (problem.compare(step, 3, "cos") == 0)
		{
			stack_.emplace(0, kCos, kHigh);
			step += 2;
		}
		else if (problem.compare(step, 3, "sin") == 0)
		{
			stack_.emplace(0, kSin, kHigh);
			step += 2;
		}
		else if (problem.compare(step, 3, "tan") == 0)
		{
			stack_.emplace(0, kTan, kHigh);
			step += 2;
		}
		else if (problem.compare(step, 4, "acos") == 0)
		{
			stack_.emplace(0, kAcos, kHigh);
			step += 3;
		}
		else if (problem.compare(step, 4, "asin") == 0)
		{
			stack_.emplace(0, kAsin, kHigh);
			step += 3;
		}
		else if (problem.compare(step, 4, "atan") == 0)
		{
			stack_.emplace(0, kAtan, kHigh);
			step += 3;
		}
		else if (problem.compare(step, 2, "ln") == 0)
		{
			stack_.emplace(0, kLn, kHigh);
			step += 1;
		}
		else if (problem.compare(step, 4, "sqrt") == 0)
		{
			stack_.emplace(0, kSqrt, kHigh);
			step += 3;
		}
		else if (problem.compare(step, 3, "log") == 0)
		{
			stack_.emplace(0, kLog, kHigh);
			step += 2;
		}
		else if (problem[step] == '(')
		{
			stack_.emplace(0, kOpenBrackets, kBracket);
		}
		else if (problem[step] == ')')
		{
			stack_.emplace(0, kCloseBrackets, kBracket);
		}
	}
}
...
void CalculatorModel::Calculate(std::stack<Token>& numbers, int operation)
{
	if (!numbers.empty())
	{
		...
		if (calc.operation < 101) // Identify Arithmetic or Trigonometric calc
		{
		...
		else
		{
			calc.left_operand = numbers.top().value;
			numbers.pop();
			calc.UnaryCalculation();
		}
		numbers.emplace(calc.result, kNumber, kLow);
	}
}

void CalculatorModel::ReplaceBrackets(std::string& problem)
{
	std::ranges::replace(problem, '[', '(');
	std::ranges::replace(problem, ']', ')'); // ranges
	std::replace(problem.begin(), problem.end(), '{', '('); //classic. Why not ^_^
	std::replace(problem.begin(), problem.end(), '}', ')');
}
